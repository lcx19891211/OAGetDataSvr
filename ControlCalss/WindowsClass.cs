...
namespace ControlClass
{
    public class WindowsClass
    {
        ...
        public static extern IntPtr FindWindowEx(
            IntPtr hwndParent, //要查找子窗口的父窗口句柄,如果hwnjParent为NULL，则函数以桌面窗口为父窗口，查找桌面窗口的所有子窗口
            IntPtr hwndChildAfter, //子窗口句柄。查找从在Z序中的下一个子窗口开始。子窗口必须为hwndPareRt窗口的直接子窗口而非后代窗口。如果HwndChildAfter为NULL，查找从hwndParent的第一个子窗口开始。如果hwndParent 和 hwndChildAfter同时为NULL，则函数查找所有的顶层窗口及消息窗口。
            string lpszClass, //指向一个指定了类名的空结束字符串，或一个标识类名字符串的成员的指针。如果该参数为一个成员，则它必须为前次调用theGlobaIAddAtom函数产生的全局成员。该成员为16位，必须位于lpClassName的低16位，高位必须为0。
            string lpszWindow //指向一个指定了窗口名（窗口标题）的空结束字符串。如果该参数为 NULL，则为所有窗口全匹配。返回值：如果函数成功，返回值为具有指定类名和窗口名的窗口句柄。如果函数失败，返回值为NULL。
            );
        /// <summary>
        /// 按窗口名称获取窗体句柄
        /// </summary>
        ...
        [DllImport("user32.dll", CharSet = CharSet.Auto, ExactSpelling = true)]
        public static extern IntPtr GetActiveWindows();
        /// <summary>
        /// 遍历父窗体下的子窗体
        /// </summary>
        /// <param name="hWndParent"></param>
        ...
        [DllImport("User32.dll", CharSet = CharSet.Auto)]
        public static extern int GetWindowText(int hWnd, StringBuilder text, int nMaxCount);
        ...
        public const int AW_HOR_POSITIVE = 0x0001;//自左向右显示窗口,该标记可以在迁移转变动画和滑动动画中应用。应用AW_CENTER标记时忽视该标记
        public const int AW_HOR_NEGATIVE = 0x0002;//自右向左显示窗口,该标记可以在迁移转变动画和滑动动画中应用。应用AW_CENTER标记时忽视该标记
        public const int AW_VER_POSITIVE = 0x0004;//自顶向下显示窗口,该标记可以在迁移转变动画和滑动动画中应用。应用AW_CENTER标记时忽视该标记
        public const int AW_VER_NEGATIVE = 0x0008;//自下向上显示窗口,该标记可以在迁移转变动画和滑动动画中应用。应用AW_CENTER标记时忽视该标记该标记
        public const int AW_CENTER = 0x0010;//若应用了AW_HIDE标记,则使窗口向内重叠;不然向外扩大
        public const int AW_HIDE = 0x10000;//隐蔽窗口
        public const int AW_ACTIVE = 0x20000;//激活窗口,在应用了AW_HIDE标记后不要应用这个标记
        public const int AW_SLIDE = 0x40000;//应用滑动类型动画结果,默认为迁移转变动画类型,当应用AW_CENTER标记时,这个标记就被忽视
        public const int AW_BLEND = 0x80000;//应用淡入淡出结果
        public const int WM_COPY = 0x301;//复制
        public const int WM_CUT = 0x300;//剪贴
        public delegate bool CallBack(IntPtr hwnd, int lParam);//托管返回事件
        public const int WM_GETTEXT = 0xD;
        #endregion

        #region WINDOWS Dll 加载
        ...
        //勾子程序处理事件
        public HookProc _KeyBoardHookProcedure;

        public IntPtr _hookWindowPtr = IntPtr.Zero;

        //外部调用的键盘处理事件
        public static ProcessKeyHandle _clientMethod = null;
        #endregion
        ...
        public void InstallHook(ProcessKeyHandle clientMethod)
        {
            ...
            if (_hHookValue == 0)
            {
                _KeyBoardHookProcedure = new HookProc(GetHookProc);
